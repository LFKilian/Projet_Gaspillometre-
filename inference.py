...
CONFIG_DIR = PROJECT_ROOT / "Final" / "config"
MODELS_DIR = PROJECT_ROOT / "Final" / "models"
...
class WeightEstimator:
    ...
    def estimate_weight(self, class_name: str, bbox_area_ratio: float) -> float:
        ...
        # Poids estimé
        # Facteur 0.85 : la bbox contient ~85% d'aliment en moyenne
        weight_g = area_cm2 * density * 0.85
...
def run_camera(detector: WasteDetector, camera_id: int = 0):
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            log_detection(result, f"capture_{ts}.jpg", log_file, resultats_file)
...
def main():
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    log_file = LOGS_DIR / "logs.jsonl"
    resultats_file = RESULTS_DIR / "resultats.jsonl"
    ...
    if args.camera is not None:
        ...
        if not image_path.exists():
            ...
            sys.exit(1)
        process_image(detector, image_path, output_dir, log_file, resultats_file)
